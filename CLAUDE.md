...
**Вербы и ручки ЗАПУСКА смоуком не трогать** (`tide return`, `tide spark`,
`/resume`, `/spark`, `/take`): они поднимают настоящую сессию в настоящем
проекте на машине человека. Выдуманный sid не защищает — `return` на него
отвечает `resumed` и спавнит терминал (кандидат 230). Проверять только резолв
в памяти: позвать функцию, которая ищет путь или нить, и посмотреть, что она
вернула. Живой запуск — рукой человека, не тестом.

**Чужое рабочее дерево не трогать никогда.** В этом репозитории одновременно
работают несколько сессий, и все — БЕЗ коммитов: «не коммитить» значит, что
несохранённая правка соседа живёт только в рабочем дереве, и `git checkout .`,
`git reset --hard` или «приведу к HEAD перед релизом» стирают её без следа.
Так уже пропали чинёный `seed_new_install`, проводка `tide board` и две строки
этого файла — в историю не попало ничего, восстанавливать пришлось по памяти.
Нужно чистое дерево под выкат — попроси соседа убрать своё; он уберёт за
минуту и вернёт после.
